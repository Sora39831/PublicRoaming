...
class SplashHook(classLoader: ClassLoader) : BaseHook(classLoader) {
    override fun startHook() {
        ...
        // v9.1.0+: BrandShowInfo 可能变成了内部类或改名，回退模式兼容
        instance.splashInfoClass?.hookAfterMethod(
            "getMode"
        ) { param -> if (fullSplash) param.result = "full" } ?: run {
            // 尝试查找包含 getMode 方法的 BrandShowInfo 相关类
            runCatching {
                val brandShowInfoClass = "tv.danmaku.bili.ui.splash.brand.model.BrandShowInfo".findClassOrNull(mClassLoader)
                    ?: "tv.danmaku.bili.ui.splash.brand.model.BrandShowInfo\$a".findClassOrNull(mClassLoader)
                brandShowInfoClass?.declaredMethods?.firstOrNull { it.name == "getMode" }?.hookAfterMethod { param ->
                    if (fullSplash) param.result = "full"
                }
            }
        }
